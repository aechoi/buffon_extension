...
for all $i>2$. Taking $g(\vec\phi)$ as the integrand
\begin{equation}
	\int_0^{S_i}g(\vec\phi)dx_i = S_ig(\vec\phi) \label{eq:irrelevant translational}
\end{equation}
...
\begin{equation}
	P_{N=1}(c) = \frac{2\Gamma(\frac{D}{2})}{\pi^{1/2}S_1\Gamma(\frac{D-1}{2})} \iint_{\phi_1,x_1}\sin^{D-2}\phi_1 dx_1 d\phi_1
\end{equation}
...
in \S \ref{s:short needle} and \S \ref{s:long needle} respectively.
